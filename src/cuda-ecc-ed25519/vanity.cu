	...
	for (int attempts = 0; attempts < ATTEMPTS_PER_EXECUTION; ++attempts) {
		...
		for (int i = 0; i < sizeof(prefixes) / sizeof(prefixes[0]); ++i) {
			for (int j = 0; j < prefix_letter_counts[i]; ++j) {
				...
				char prefix_char = tolower(prefixes[i][j]);
				char key_char = tolower(key[j]);

				// Check if they match, considering '?' as a wildcard
				if (!(prefix_char == '?' || prefix_char == key_char)) {
					break;  // No match for this prefix, go to the next one
				}

				// If the end of the prefix is reached and all matched, we found a key
				if (j == (prefix_letter_counts[i] - 1)) {
					...
					break;  // Prefix matched, exit this loop
				}
			}
		}
